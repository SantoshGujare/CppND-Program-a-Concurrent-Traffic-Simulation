...
T MessageQueue<T>::receive()
{
    ...
    std::unique_lock<std::mutex> unique_lck(_qmutex);
    _qcondVar.wait(unique_lck,[this]{return !_queue.empty();});
...
void TrafficLight::cycleThroughPhases()
{
    ...
    int cycleDuration = distributor(randEng);
    auto startTime = std::chrono::high_resolution_clock::now();
  	while(true)
    {
      auto currentTime = std::chrono::high_resolution_clock::now();
      auto elaspedTime = std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - startTime).count();
      if(elaspedTime > cycleDuration)
      {
        ...
        cycleDuration = distributor(randEng);
      }
